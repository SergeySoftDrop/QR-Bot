...
export default new Middleware(
    async(ctx, next) => {
        if(!ctx.state.lastAdminChek) ctx.state.isAdmin = await chek(ctx);
        else if(Date.now() - ctx.state.lastAdminChek <= conf.chekRate) ctx.state.isAdmin = await chek(ctx);
