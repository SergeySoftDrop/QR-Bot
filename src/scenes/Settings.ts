...
const getButtons = async(adminId: number): Promise<TelegramBot.InlineKeyboardButton[][]> => {
...
const sendSettings = async(ctx: State) => {
...
const enable = async(adminId: number, field: string) => {
    const update = { [`notifySettings.${field}`]: true };
    await Admin.findOneAndUpdate({ userId: adminId }, { $set: update });
};

const disable = async(adminId: number, field: string) => {
    const update = { [`notifySettings.${field}`]: false };
    await Admin.findOneAndUpdate({ userId: adminId }, { $set: update });
};

const handle = async(ctx: State, field: string, action: 'enable' | 'disable') => {
    switch(action) {
        case 'enable':
...
const scene = new Scene('settings', 
    async(ctx) => {
        if(!RequestHelper.isCallback(ctx) || !RequestHelper.callbackDataIs(ctx, /(leave|newReports_.*|suspiciousActivity_.*)/)) return await invalidDataAnswer(ctx);

        const callbackData = ctx.data!.data;
        if(
            callbackData.startsWith('newReports_') || 
            callbackData.startsWith('suspiciousActivity_')
        ) {
            const [field, action] = callbackData.split('_'); 
            await handle(ctx, field, action as 'enable' | 'disable');
