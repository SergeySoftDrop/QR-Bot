...
const invalidData = async(ctx: State, type: 'data'|'param') => {
    switch(type) {
        case 'data':
...
const scene = new Scene('create',
    async(ctx) => {
        if(!RequestHelper.isCallback(ctx) || !RequestHelper.callbackDataIs(ctx, 'default', 'custom', 'leave')) return await invalidData(ctx, 'data');
    ...
    async(ctx) => {
        if(!RequestHelper.isText(ctx)) return await invalidData(ctx, 'param');

        const size = Number.parseInt(ctx.data!.text);
        if(isNaN(size) || size < 10 || size > 1000) return await invalidData(ctx, 'param');
    ...
    async(ctx) => {
        if(!RequestHelper.isText(ctx) || !isValidRGB(ctx.data!.text)) return await invalidData(ctx, 'param');
    ...
    async(ctx) => {
        ...
        if(isNaN(qzone) || qzone < 0 || qzone > 100) return await invalidData(ctx, 'param');
    ...
    async(ctx) => {
        if(!RequestHelper.isCallback(ctx) || !RequestHelper.callbackDataIs(ctx, 'generate', 'again')) return await invalidData(ctx, 'data');
    ...
    async(ctx) => {
        if(!RequestHelper.isCallback(ctx) || !RequestHelper.callbackDataIs(ctx, 'leave', 'again')) return await invalidData(ctx, 'data');
...
scene.leave(async(ctx) => {
    if(ctx.state.generate) delete ctx.state.generate;
