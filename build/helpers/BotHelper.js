...
class BotHelper {
    static setScenes(bot) {
        bot.setScene(reportScene, viewReportScene, settingsScene, readScene, createScene);
    }
    static setBotCommands(bot) {
        bot.command('start', async (ctx) => {
            await ctx.api.reply(`<b>Добро пожаловать!</b>\n\n` +
                `Что умеет наш бот:\n` +
        ...
        bot.command('help', async (ctx) => {
            await ctx.api.reply(`Список доступных команд:\n` +
        ...
        bot.command('view_report', async (ctx) => {
            if (AdminHelper.isAdmin(ctx))
                await ctx.sceneWorker.enter('viewReports');
        });
        bot.command('settings', async (ctx) => {
            if (AdminHelper.isAdmin(ctx))
                await ctx.sceneWorker.enter('settings');
        });
        bot.command('ban', async (ctx, args) => {
            if (AdminHelper.isAdmin(ctx)) {
                if (args.length < 3)
                    return await ctx.api.reply('Отправьте ID пользователя и причину после команды /ban');
        ...
        bot.command('unban', async (ctx, args) => {
            if (AdminHelper.isAdmin(ctx)) {
                if (args.length < 2)
                    return await ctx.api.reply('Отправьте бан-ID пользователя /unban');
                await AdminHelper.delBan(ctx, args[1]);
            }
        });
    }
    static setMiddlewares(bot) {
        bot.use(observeMiddleware, SetAdminMiddleware, BanMiddleware);
    }
}
export default BotHelper;
//# sourceMappingURL=BotHelper.js.map