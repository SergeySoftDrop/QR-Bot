...
const updateBanState = async (ctx) => {
...
export default new Middleware(async (ctx, next) => {
    let ban;
    if (!ctx.state.banState || ctx.state.banState.lastChek + conf.checkRate <= Date.now())
        ban = await updateBanState(ctx);
    if (ctx.state.banState.isBaned) {
        if (!ctx.state.banState.msgSended) {
            await ctx.api.reply(`<b>Вы были заблокированы.</b>\n` +
                ...
                `Укажите ID бана в обращении.`, { parse_mode: 'HTML' });
...
//# sourceMappingURL=BanMiddleware.js.map