...
const scene = new Scene('report', async (ctx) => {
    if (!RequestHelper.isCallback(ctx) || !RequestHelper.callbackDataIs(ctx, 'start', 'leave'))
        return await invalidData(ctx);
...
scene.command('leave', async (ctx) => {
    if (ctx.state.messageForEdit)
        await ctx.api.telegram.deleteMessage(ctx.data.chat.id, ctx.state.messageForEdit);
    await ctx.sceneWorker.leave();
});
scene.enter(async (ctx) => {
    ctx.state.messageForEdit = (await ctx.api.reply('Создать сообщение об ошибке в боте?', {
        reply_markup: {
            inline_keyboard: [
