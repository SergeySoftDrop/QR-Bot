...
const scene = new Scene('read', async (ctx) => {
    if (!RequestHelper.isText(ctx) && !RequestHelper.hasPhoto(ctx))
        return await invalidData(ctx, 'general');
    const msgIdForEdit = (await ctx.api.reply('Обработка...')).message_id;
    let url;
    if (RequestHelper.isText(ctx)) {
        if (!isValidUrl(ctx.data.text))
            return await invalidData(ctx, 'url');
        url = ctx.data.text;
    }
    ...
    if (!res[0].symbol[0].data)
        return await invalidData(ctx, 'url-image');
    ...
    if (!RequestHelper.isCallback(ctx) || !RequestHelper.callbackDataIs(ctx, 'send_agin', 'leave'))
        return await invalidData(ctx, 'general');
...
//# sourceMappingURL=ReadQR.js.map